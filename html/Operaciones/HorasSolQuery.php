...
include('../Configuraciones/bd.php');
    ...
    $diaActual = date('z'); 
                                                                    //$horaActual_prim es la hora actual menos 10 minutos.

    $query = "SELECT * FROM `tb_dataday` WHERE ID like '$diaActual'";                                      //Definimos la Query con la variable recibida (el % es para buscar todos los valores no solo el primero)
    ...
    while($row = mysqli_fetch_array($result)){
        $json[] = array(
            'Amanecer' => $row['Amanecer'],
            'Anochecer' => $row['Anochecer']
        );
    }
